...
public class StudentServiceImpl implements StudentService {
    ...
    public Pair<Double, Integer> addTask(AddTaskDTO addTaskDTO, Long studentId) {
        ...
        if (relation.isEmpty()) {
            ...
            Task task = taskService.findByTitleAndLink(addTaskDTO.getTitle(), addTaskDTO.getLink())
            ...
            StudentTask studentTask = studentTaskRepository.findById(relation.get().getId())
                    .orElseThrow(RuntimeException::new);
            studentTask.setVerdict(addTaskDTO.getVerdict());
            studentTask.setLastChangedTime(LocalDateTime.now());
            studentTaskRepository.save(studentTask);
    ...
    public List<Pair<Double, Integer>> syncWithAcmp(Long studentId) {
        System.out.println(acmpService.getRawHtmlUserPage("228"));
        return null;
//        Student student = studentRepository.findById(studentId)
//                .orElseThrow(() -> new EntityNotFoundException("Student not found"));
